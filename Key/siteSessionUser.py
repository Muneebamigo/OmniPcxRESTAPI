...
# Update the sessionkeysiteUser in the config.py file
if token:
    # Open the config.py file in append mode to update the sessionkeysiteUser
    config_file_path = 'config.py'
    with open(config_file_path, 'r') as file:
        config_content = file.read()

    # Replace or insert the sessionkeysiteUser value into the config.py content
    if 'sessionkeysiteUser' in config_content:
        # If sessionkeysiteUser already exists, replace its value
        config_content = config_content.replace(
            f'sessionkeysiteUser = "{config.sessionkeysiteUser}"',
            f'sessionkeysiteUser = "{token}"'
        )
    else:
        # If sessionkeysiteUser doesn't exist, add it to the file
        config_content += f'\nsessionkeysiteUser = "{token}"\n'

    # Write the updated content back to the config.py file
    with open(config_file_path, 'w') as file:
        file.write(config_content)

    print(f"sessionkeysiteUser has been saved to {config_file_path}")
else:
